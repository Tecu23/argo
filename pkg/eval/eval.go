...
var (
	ClearRank = [8]Bitboard{}
	ClearFile = [8]Bitboard{}
	MaskRank  = [8]Bitboard{}
	MaskFile  = [8]Bitboard{}
)
...
type KingZone struct {
	OuterRing bitboard.Bitboard
	InnerRing bitboard.Bitboard
}

var (
	KingZones         [64]KingZone
	DoubledPawnMasks  [2][64]bitboard.Bitboard
	IsolatedPawnMasks [8]bitboard.Bitboard
	PassedPawnMasks   [2][64]bitboard.Bitboard
	OutpostMasks      [2][64]bitboard.Bitboard
)

var (
	PieceValueMG = [6]int16{84, 333, 346, 441, 921}
	PieceValueEG = [6]int16{106, 244, 268, 478, 886}
)

var (
	PieceMobilityMG = [5]int16{0, 5, 3, 3, 0}
	PieceMobilityEG = [5]int16{0, 2, 3, 2, 6}
)

var (
	BishopPairBonusMG int16 = 22
	BishopPairBonusEG int16 = 30
)

var (
	IsolatedPawnPenatlyMG int16 = 17
	IsolatedPawnPenatlyEG int16 = 6
	DoubledPawnPenatlyMG  int16 = 1
	DoubledPawnPenatlyEG  int16 = 16
)
...
var (
	KnightOnOutpostBonusMG int16 = 27
	KnightOnOutpostBonusEG int16 = 18
)

var (
	RookOnOpenFileBonusMG int16 = 23
	TempoBonusMG          int16 = 14
)

var (
	BishopOutPostBonusMG int16 = 10
	BishopOutPostBonusEG int16 = 14
)

var (
	OuterRingAttackPoints               = [5]int16{0, 1, 0, 1, 1}
	InnerRingAttackPoints               = [5]int16{0, 3, 4, 3, 2}
	SemiOpenFileNextToKingPenalty int16 = 4
)
...
func EvaluatePos(b *board.Board) int16 {
	if isDrawn(b) {
		return Draw
	}
	...
	score := int16(
		((int32(mgScore) * (int32(256) - int32(phase))) + (int32(egScore) * int32(phase))) / int32(
			256,
		),
	)
...
func evalKnight(pos *Position, color, sq uint8, eval *Eval) {
	...
	if outerRingAttacks != 0 || innerRingAttacks != 0 {
		...
		eval.KingAttackPoints[color] += uint16(
			outerRingAttacks.CountBits(),
		) * uint16(
			OuterRingAttackPoints[Knight],
		)
		eval.KingAttackPoints[color] += uint16(
			innerRingAttacks.CountBits(),
		) * uint16(
			InnerRingAttackPoints[Knight],
		)
	}
}
...
func evalBishop(pos *Position, color, sq uint8, eval *Eval) {
	...
	if outerRingAttacks != 0 || innerRingAttacks != 0 {
		...
		eval.KingAttackPoints[color] += uint16(
			...
			OuterRingAttackPoints[Bishop],
		)
		eval.KingAttackPoints[color] += uint16(
			innerRingAttacks.CountBits(),
		) * uint16(
			InnerRingAttackPoints[Bishop],
		)
	}
}
...
func evalRook(pos *Position, color, sq uint8, eval *Eval) {
	...
	if outerRingAttacks != 0 || innerRingAttacks != 0 {
		...
		eval.KingAttackPoints[color] += uint16(
			...
			OuterRingAttackPoints[Rook],
		)
		eval.KingAttackPoints[color] += uint16(
			innerRingAttacks.CountBits(),
		) * uint16(
			InnerRingAttackPoints[Rook],
		)
	}
}
...
func evalQueen(pos *Position, color, sq uint8, eval *Eval) {
	...
	if outerRingAttacks != 0 || innerRingAttacks != 0 {
		...
		eval.KingAttackPoints[color] += uint16(
			...
			OuterRingAttackPoints[Queen],
		)
		eval.KingAttackPoints[color] += uint16(
			innerRingAttacks.CountBits(),
		) * uint16(
			InnerRingAttackPoints[Queen],
		)
	}
}
...
func InitEvalBitboards() {
	...
	for sq := 0; sq < 64; sq++ {
		...
		KingZones[sq] = KingZone{
			OuterRing: zone & ^(KingMoves[sq] | sqBB),
			InnerRing: KingMoves[sq] | sqBB,
		}
